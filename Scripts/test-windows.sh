...
# Usage:
#   ./test-windows.sh
#   ./test-windows.sh --filter FullyQualifiedName~TraderStockPatch
...
MAIN_PROJECT='Source\1.6\BionicThumbGuild.csproj'
TEST_PROJECT='Tests\1.6\BionicThumbGuild.Tests.csproj'

echo -e "${BLUE}=== Bionic Thumb Guild - Windows Test Runner ===${NC}"
...
#    Stage 3 robocopies the test bin to %TEMP%\btg-tests\ — a real local
...
powershell.exe -NoProfile -Command "
    ...
    \$LocalBin = Join-Path \$env:TEMP 'btg-tests'
    ...
    dotnet test 'BionicThumbGuild.Tests.dll' --nologo $EXTRA_ARGS
    exit \$LASTEXITCODE
" 2>&1 | tee "$TMPF"
STATUS=${PIPESTATUS[0]}
