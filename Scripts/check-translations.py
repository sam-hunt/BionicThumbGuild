...
# final path segment. This repo has none; the constant exists so the sibling
# repos' checkers stay structurally identical. The sidecar's fullListAllowed
# flag could eventually subsume it.
PARITY_EXEMPT_FIELDS = set()

# DLCs the sidecar must have been generated with. This mod requires no DLC
# (About.xml has no DLC modDependencies) and gates no content behind
# MayRequire, so the set is empty.
REQUIRED_DLCS = set()
...
def check_language(lang_dirs, english_keyed, sidecar, def_roots, report):
    ...
    # can only live under its own compat root's Languages/<Lang>/ (MayRequire
    # is ignored on DefInjected, so the LoadFolders gate has to be the
    # folder), yet the sidecar may mark them required — checking that root
    # standalone would demand the whole Keyed set and every other def's
    # injections from it. This repo has no compat roots today, but the
    # mechanism must hold the moment one is added.
