...
# that reshuffles our surgery recipe or hediff comps would show up in the
...
# refresh flow); like shunter.tradersstockxenogerms, this mod is not part of
# a probed family — it is the only sidecar-bearing mod in its own boot.
PACKAGE_ID = "shunter.bionicthumbguild"
...
# Membership rule:
#   * Core. This mod requires no DLC (About.xml has no DLC
#     modDependencies) and gates no content behind MayRequire, so no DLC
#     belongs in the pinned list;
#   * hard dependencies (Harmony);
#   * this repo's own sidecar-bearing mod. This mod is not part of a probed
#     family (unlike the UniqueMeleeWeapons trio riding along in one boot
#     for convenience), so the list has no siblings to add;
#   * a third-party mod ONLY if this mod's content MayRequires it. Nothing
#     currently does — the mod ships no cross-mod patches — so none is
#     listed today.
